...
def create_emotion_dataloaders(logger, cfg) -> Dict[str, DataLoader]:
    """
    ...
    seen_emotions = dict(task.seen_emotions)
    unseen_emotions = dict(task.unseen_emotions) if task.unseen_emotions else {}
    train_ratio = task.train_ratio
    merge_train_dev = task.merge_train_dev
    ...
    logger.info(f"  Seen emotions: {list(seen_emotions.keys())} -> {list(range(len(seen_emotions)))}")
    if unseen_emotions:
        logger.info(f"  Unseen emotions: {list(unseen_emotions.keys())} -> {list(range(unseen_start_label, unseen_start_label + len(unseen_emotions)))}")
    ...
    for emotion, label_id in seen_emotions.items():
        logger.info(f"  加载 {emotion} (原始label={label_id}, 新label={emotion_label_map[label_id]})...")
    ...
    logger.info(f"\n【数据集统计】")
    logger.info(f"  训练集总样本数: {len(train_dataset)}")
    logger.info(f"  测试集总样本数: {len(test_dataset)}")

    train_stats = train_dataset.get_stats()
    logger.info(f"  训练集情感分布: {train_stats['emotion_counts']}")
    logger.info(f"  Seen样本数: {train_stats['seen_counts']}, Unseen样本数: {train_stats['unseen_counts']}")
