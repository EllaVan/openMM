...
def extract_iemocap(config):
    ...
    emotion_name_map = {
        0: 'happy', 1: 'sad', 2: 'surise', 3: 'disgust',
        4: 'anger', 5: 'fear', 6: 'frustrated', 7: 'excited', 8: 'neutral'
    }
    ...
    for emotion, samples in emotion_data.items():
        if len(samples) > 0:
            ...
            output_file = os.path.join(output_dir, f"IEMOCAP_{emotion}.pkl")#f"IEMOCAP_{emotion}label{label}.pkl"
            with open(output_file, 'wb') as f:
                pickle.dump(samples, f)
            stats[emotion] = len(samples)
            logger.info(f"✓ {emotion}: {len(samples)} 样本 -> {output_file}")
