"""
Whitebox Bayesian Continual Learning - Main Execution Script

Complete Task 0 to Task T training with interpretable Beta-Bernoulli AU-EMO matrix.
...
from beta_bernoulli_matrix import BetaBernoulliAUEMOMatrix, load_au_emo_prior
from em_trainer import EMTrainerWhitebox
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Whitebox Bayesian Continual Learning'
    )
    ...
    parser.add_argument('--prior_strength', type=float, default=100.0,
                       help='Beta-Bernoulli prior strength')
    ...
    parser.add_argument('--num_em_iterations', type=int, default=3,
                       help='Number of EM iterations per epoch')
    ...
    parser.add_argument('--seen_update_weight', type=float, default=1.0)
    parser.add_argument('--unseen_update_weight', type=float, default=0.8)
    parser.add_argument('--au_emo_regularization', type=float, default=0.01)

    # Output
    parser.add_argument('--save_dir', type=str,
                       default='../../checkpoints/whitebox')
...
def main():
    ...
    print("\n" + "="*80)
    print("WHITEBOX BAYESIAN CONTINUAL LEARNING")
    print("="*80)
    print("Framework: Beta-Bernoulli AU-EMO Matrix with EM Algorithm")
    ...
    # ===== 4. Create Beta-Bernoulli AU-EMO Matrix =====
    print("\n" + "-"*80)
    print("Creating Beta-Bernoulli AU-EMO Matrix...")
    print("-"*80)
    ...
    print(f"Matrix initialized with Beta-Bernoulli prior")
    print(f"Prior strength: {args.prior_strength}")
    print("\nInitial P(AU|EMO) statistics:")
    print(f"  Mean: {au_emo_matrix.get_p_au_given_emo().mean().item():.4f}")
    print(f"  Mean uncertainty: {au_emo_matrix.get_uncertainty().mean().item():.6f}")
    ...
    model = AUEmotionNetwork(
        ...
        num_aus=args.num_aus,
        num_emotions=num_emotions,
        au_emo_prior=None,  # We use external Beta-Bernoulli matrix
    ...
    # Replace model's internal matrix with our Beta-Bernoulli matrix
    ...
    print(f"EM Trainer initialized")
    print(f"  EWC: {args.use_ewc} (lambda: {args.ewc_lambda})")
    print(f"  Consistency: {args.consistency_strategy}")
    print(f"  Update weights: seen={args.seen_update_weight}, unseen={args.unseen_update_weight}")
    ...
    # ===== 9. Training Loop =====
    print("\n" + "="*80)
    print("STARTING WHITEBOX EM TRAINING")
    print("="*80)
    ...
    with open(save_dir / 'training_summary.txt', 'w') as f:
        f.write("WHITEBOX BAYESIAN CONTINUAL LEARNING\n")
        f.write("="*80 + "\n\n")
        f.write(trainer.get_training_summary())
        f.write("\n\n")
        f.write(metrics.get_summary())
        f.write("\n\n")
        f.write("Final AU-EMO Matrix:\n")
        f.write(au_emo_matrix.visualize_matrix(
            au_names=au_names,
            emotion_names=emotion_names,
            show_uncertainty=True
    ...
    print(f"\n{'='*80}")
    print("WHITEBOX TRAINING COMPLETED!")
